give @s minecraft:arrow[item_model="surviwar:notice",minecraft:item_name="",tooltip_style="surviwar:tip",minecraft:lore=[\
                        {text:""},\
                        {text:""},\
                        {text:"\uE000","font":"surviwar:item",italic:false,color:white},\
                        [{"atlas":"gui","sprite":"surviwar:warning_tape",color:white,shadow_color:0},{"atlas":"gui","sprite":"surviwar:warning_tape",color:white,shadow_color:0},{"atlas":"gui","sprite":"surviwar:warning_tape",color:white,shadow_color:0},{"atlas":"gui","sprite":"surviwar:warning_tape",color:white,shadow_color:0},{"atlas":"gui","sprite":"surviwar:warning_tape",color:white,shadow_color:0},{"atlas":"gui","sprite":"surviwar:warning_tape",color:white,shadow_color:0},{"atlas":"gui","sprite":"surviwar:warning_tape",color:white,shadow_color:0},{"atlas":"gui","sprite":"surviwar:warning_tape",color:white,shadow_color:0},{"atlas":"gui","sprite":"surviwar:warning_tape",color:white,shadow_color:0},{"atlas":"gui","sprite":"surviwar:warning_tape",color:white,shadow_color:0},{"atlas":"gui","sprite":"surviwar:warning_tape",color:white,shadow_color:0},{"atlas":"gui","sprite":"surviwar:warning_tape",color:white,shadow_color:0},{"atlas":"gui","sprite":"surviwar:warning_tape",color:white,shadow_color:0},{"atlas":"gui","sprite":"surviwar:warning_tape",color:white,shadow_color:0},{"atlas":"gui","sprite":"surviwar:warning_tape",color:white,shadow_color:0},{"atlas":"gui","sprite":"surviwar:warning_tape",color:white,shadow_color:0},{"atlas":"gui","sprite":"surviwar:warning_tape",color:white,shadow_color:0},{"atlas":"gui","sprite":"surviwar:warning_tape",color:white,shadow_color:0},{"atlas":"gui","sprite":"surviwar:warning_tape",color:white,shadow_color:0},{"atlas":"gui","sprite":"surviwar:warning_tape",color:white,shadow_color:0},{"atlas":"gui","sprite":"surviwar:warning_tape",color:white,shadow_color:0},{"atlas":"gui","sprite":"surviwar:warning_tape",color:white,shadow_color:0},{"atlas":"gui","sprite":"surviwar:warning_tape",color:white,shadow_color:0},],\
                        [{text:"·",italic:false,color:white},{player:{name:"Aurelith_FW"},color:white},{translate:"sw.info.datapack_auther",color:"aqua"}],\
                        [{text:"·",italic:false,color:white},{player:{name:"Aurelith_FW"},color:white},{translate:"sw.info.resourcepack_auther",color:"aqua"}],\
                        [{text:"·",italic:false,color:white},{player:{name:"Aurelith_FW"},color:white},{translate:"sw.info.lobby_bulider",color:"aqua"}],\
                        [{text:"·",italic:false,color:white},{player:{name:"Aurelith_FW"},color:white},{translate:"sw.info.arena_bulider",color:"aqua"}],\
                        [{text:"\uE005","font":"surviwar:item",italic:false,color:white},{translate:"sw.info.version",font:"default",color:yellow}],\
                        [{"atlas":"gui","sprite":"surviwar:warning_tape",color:white,shadow_color:0},{"atlas":"gui","sprite":"surviwar:warning_tape",color:white,shadow_color:0},{"atlas":"gui","sprite":"surviwar:warning_tape",color:white,shadow_color:0},{"atlas":"gui","sprite":"surviwar:warning_tape",color:white,shadow_color:0},{"atlas":"gui","sprite":"surviwar:warning_tape",color:white,shadow_color:0},{"atlas":"gui","sprite":"surviwar:warning_tape",color:white,shadow_color:0},{"atlas":"gui","sprite":"surviwar:warning_tape",color:white,shadow_color:0},{"atlas":"gui","sprite":"surviwar:warning_tape",color:white,shadow_color:0},{"atlas":"gui","sprite":"surviwar:warning_tape",color:white,shadow_color:0},{"atlas":"gui","sprite":"surviwar:warning_tape",color:white,shadow_color:0},{"atlas":"gui","sprite":"surviwar:warning_tape",color:white,shadow_color:0},{"atlas":"gui","sprite":"surviwar:warning_tape",color:white,shadow_color:0},{"atlas":"gui","sprite":"surviwar:warning_tape",color:white,shadow_color:0},{"atlas":"gui","sprite":"surviwar:warning_tape",color:white,shadow_color:0},{"atlas":"gui","sprite":"surviwar:warning_tape",color:white,shadow_color:0},{"atlas":"gui","sprite":"surviwar:warning_tape",color:white,shadow_color:0},{"atlas":"gui","sprite":"surviwar:warning_tape",color:white,shadow_color:0},{"atlas":"gui","sprite":"surviwar:warning_tape",color:white,shadow_color:0},{"atlas":"gui","sprite":"surviwar:warning_tape",color:white,shadow_color:0},{"atlas":"gui","sprite":"surviwar:warning_tape",color:white,shadow_color:0},{"atlas":"gui","sprite":"surviwar:warning_tape",color:white,shadow_color:0},{"atlas":"gui","sprite":"surviwar:warning_tape",color:white,shadow_color:0},{"atlas":"gui","sprite":"surviwar:warning_tape",color:white,shadow_color:0},],\
                        [{text:"           >>>",color: "gold",italic:false},{translate:"sw.info.extra",color: "green"},{text:"<<<",color:"gold"}]],\
                        minecraft:tooltip_display={hidden_components:["item_model"]}]

give @s minecraft:angler_pottery_sherd[item_model="surviwar:yes",item_name={translate:"sw.gui.button.ready",color:"green"},lore=[{translate:"sw.gui.button.ready.tooltip",color:"gray"}]]
give @s minecraft:archer_pottery_sherd[item_model="surviwar:no",item_name={translate:"sw.gui.button.not_ready",color:"red"},lore=[{translate:"sw.gui.button.not_ready.tooltip",color:"gray"}]]
give @s minecraft:arms_up_pottery_sherd[item_model="surviwar:lock",item_name={translate:"sw.gui.button.lock",color:"gray"}]
give @s minecraft:blade_pottery_sherd[item_model="surviwar:spectator",item_name={translate:"sw.gui.button.spectator",color:"yellow"},lore=[{translate:"sw.gui.button.spectator.tooltip",color:"gray"}]]
give @s minecraft:danger_pottery_sherd[item_model="surviwar:back",item_name={translate:"sw.gui.button.back",color:"dark_gray"},lore=[{translate:"sw.gui.button.back.tooltip",color:"gray"}]]
give @s minecraft:explorer_pottery_sherd[item_model="surviwar:player",item_name={translate:"sw.gui.button.player",color:"yellow"},lore=[{translate:"sw.gui.button.player.tooltip",color:"gray"}]]
give @s minecraft:flow_pottery_sherd[item_model="surviwar:ranteam",item_name={translate:"sw.gui.button.ranteam",color:"yellow"},lore=[{translate:"sw.gui.button.ranteam.tooltip",color:"gray"}]]
give @s minecraft:friend_pottery_sherd[item_model="surviwar:teams",item_name={translate:"sw.gui.button.teams",color:"yellow"}]
give @s minecraft:brewer_pottery_sherd[item_model="surviwar:question",item_name={translate:"sw.gui.button.help",color:"yellow"},lore=[{translate:"sw.gui.button.help.tooltip",color:"gray"}]]
give @s minecraft:paper[item_model="surviwar:empty",item_name={text:"技术物品",color:"yellow"}]

#TEAMS
#基本物品，tooltip需要动态更新
give @s minecraft:white_wool[item_name={translate:"sw.team.white",color:"white"}]
give @s minecraft:light_gray_wool[item_name={translate:"sw.team.light_gray",color:"gray"}]
give @s minecraft:gray_wool[item_name={translate:"sw.team.gray",color:"gray"}]
give @s minecraft:black_wool[item_name={translate:"sw.team.black",color:"white"}]
give @s minecraft:brown_wool[item_name={translate:"sw.team.brown",color:"gold"}]
give @s minecraft:red_wool[item_name={translate:"sw.team.red",color:"red"}]
give @s minecraft:orange_wool[item_name={translate:"sw.team.orange",color:"gold"}]
give @s minecraft:yellow_wool[item_name={translate:"sw.team.yellow",color:"yellow"}]
give @s minecraft:lime_wool[item_name={translate:"sw.team.lime",color:"green"}]
give @s minecraft:green_wool[item_name={translate:"sw.team.green",color:"dark_green"}]
give @s minecraft:cyan_wool[item_name={translate:"sw.team.cyan",color:"dark_aqua"}]
give @s minecraft:light_blue_wool[item_name={translate:"sw.team.light_blue",color:"aqua"}]
give @s minecraft:blue_wool[item_name={translate:"sw.team.blue",color:"blue"}]
give @s minecraft:purple_wool[item_name={translate:"sw.team.purple",color:"dark_purple"}]
give @s minecraft:magenta_wool[item_name={translate:"sw.team.magenta",color:"light_purple"}]
give @s minecraft:pink_wool[item_name={translate:"sw.team.pink",color:"light_purple"}]